...
function(exports, shader, framebuffer, data) {
	...
	function drawLineBresenham(startX, startY, startZ, endX, endY, endZ, color, storeIntersectionForScanlineFill, edgeStartVertexIndex, edgeEndVertexIndex, edgeStartTextureCoord, edgeEndTextureCoord) {
		...
		if(endX < startX) return drawLineBresenham(endX, endY, endZ, startX, startY, startZ, color, storeIntersectionForScanlineFill, edgeEndVertexIndex, edgeStartVertexIndex, edgeEndTextureCoord, edgeStartTextureCoord);
		...
		let dXAbs2 = 2 * dXAbs;
		let dYAbs2 = 2 * dYAbs;
		let dXdYdiff2 = 2 * (dXAbs - dYAbs);
		let dYdXdiff2 = 2 * (dYAbs - dXAbs);
		...
		if (dXAbs >= dYAbs)
		{
			...
			while (x !== endX)
			{
				...
				if(storeIntersectionForScanlineFill) {
					if(y !== previousY && y !== endY && y !== startY && startY !== endY && x !== endX)
					{
	...
	function scanlineStrokePolygon(vertices, polygon, color)
	{
		...
		for(let i = 0; i < polygon.length; i++) {
			...
			let st = vertices[polygon[i]];
			// Connect edge to next or to first vertex to close the polygon.
			let nextVertexIndex = (i < polygon.length - 1) ? i + 1 : 0;
	...
	function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
		...
		let startPoint, endPoint, lastIndex = 0;
		...
		if(! calcPlaneEquation(vertices, polygon)) return;
		...
		for(let i = polygon.length - 1; i > 0; i--)
		{
			...
			let pCalcDerivate = calcDerivative(Math.floor(startPoint[1]), Math.floor(endPoint[1]));
			if (pCalcDerivate !== 0) {
				derivative = pCalcDerivate;
				break;
			}
		}
		...
		for(let i = 0; i < polygon.length; i++) {
			...
			let edgeStartVertexIndex = startPoint;
			let edgeEndVertexIndex = endPoint;
	...
	function interpolationPrepareScanline(startIntersection, endIntersection, texture) {
		...
		let xStartFill = startIntersection.x;
		// End-point for filling on scanline.
		let xEndFill = endIntersection.x;
		// Start and end for z-interpolated.
		let zStartFill = startIntersection.z;
		let zEndFill = endIntersection.z;

		interpolationData.z = zStartFill;

		// To calculate dz.
		let deltaX = xEndFill - xStartFill;
		...
		let interpolationWeightStart = startIntersection.interpolationWeight;
		let interpolationWeightEnd = endIntersection.interpolationWeight;
		...
		if(texture != null) {
			...
			if(!interpolationData.uvVec) interpolationData.uvVec = [];

			if(!interpolationData.uvVecDelta) interpolationData.uvVecDelta = [];
	...
	function interpolationStepOnScanline(texture) {
		...
		interpolationData.z = interpolationData.z + interpolationData.dz;
	...
	function scanlineFillPolygon(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
		...
		let shadingFunction = shader.getShadingFunction();
		...
		for (let i = 0; i < scanlineIntersection.length; i++)
		{
			...
			findPairs(sorted).map((pair) => {
				drawLineBresenham(pair.minX, i, pair.startZ, pair.maxX, i, pair.endZ, color, true);
			});
		}
		...
		function findPairs(array)
		{
			...
			{
				let startX = array[i].x;
				let startZ = array[i].z;
				let currX = startX;
				let offset = 0;
				if (array[i + 1].x !== currX + 1)
				{
					offset += 1;
				} else { while (i + 1 < array.length && array[i + 1].x === currX + 1)
					{
						currX += 1;
						offset += 1;
					}
				}
				storage.push({ minX: startX, maxX: array[i + offset].x, startZ: startZ, endZ: array[i + offset].z });
				i += offset + 1;
			}
			return storage;
		}
	...
	function addIntersection(x, y, z, interpolationWeight, edgeStartVertexIndex, edgeEndVertexIndex, edgeStartTextureCoord, edgeEndTextureCoord) {
		...
		if(scanlineIntersection[y] === undefined) scanlineIntersection[y] = [];


		// Each intersection is an object, an array is not significantly faster.
		scanlineIntersection[y].push({ x : x, z : z, edgeStartVertexIndex : edgeStartVertexIndex, edgeEndVertexIndex : edgeEndVertexIndex, edgeStartTextureCoord : edgeStartTextureCoord, edgeEndTextureCoord : edgeEndTextureCoord, interpolationWeight : interpolationWeight});
