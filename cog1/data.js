...
define(["exports", "data", "glMatrix"], function(data, exports) {
	...
	function cleanData(){
		...
		let doublePolyVerticeIndices = {};
		let doubleVectors = {};
		for (let i = 0; i < this.vertices.length; i++) {
			vertexToCheck = this.vertices[i];
			...
			for (let j = i + 1; j < this.vertices.length; j++)
			{
				...
				{
					if ( !doubleVectors[vectorToKey(vertexToCheck)] || !doubleVectors[vectorToKey(vertexToCheck)].found)
					{
						let temp = [];
						temp.push(j);
						doubleVectors[vectorToKey(vertexToCheck)] = {};
						doubleVectors[vectorToKey(vertexToCheck)].original = i;
						doubleVectors[vectorToKey(vertexToCheck)].found = temp;
					} else
					{
		...
		for (let i = 0; i < this.polygonVertices.length; i++)
		{
			...
			for (let j = i + 1; j < this.polygonVertices.length; j++)
			{
				const next = this.polygonVertices[j];
				if (vectorsEqual(vertexToCheck, next))
				{
					if ( !doublePolyVerticeIndices[vectorToKey(vertexToCheck)] || !doublePolyVerticeIndices[vectorToKey(vertexToCheck)].found )
					{
						let temp = [];
						temp.push(j);
						doublePolyVerticeIndices[vectorToKey(vertexToCheck)] = {};
						doublePolyVerticeIndices[vectorToKey(vertexToCheck)].found = temp;
					} else {
	...
	function calcuateVertexNormalsFromPolygonNormals(){
		...
		for (let i = 0; i < this.polygonVertices.length; i++) {
			const polygon = this.polygonVertices[i];

			// Loop over vertices of polygon.
			for (let m = 0; m < polygon.length; m++) {

				// Accumulate/add all polygon normals.
				if (polygon[m] < this.vertexNormals.length)
					vec3.add(this.vertexNormals[polygon[m]], this.polygonNormals[i]);
				else console.log("Falscher Polygonindex", polygon[m]);
			}
		}
